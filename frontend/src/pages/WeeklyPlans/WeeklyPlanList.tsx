// frontend/src/pages/WeeklyPlans/WeeklyPlanList.tsx
...
const WeeklyPlanList: React.FC = () => {
  ...
  const [weeklyPlans, setWeeklyPlans] = useState<WeeklyPlanListItem[]>([]);
  ...
  const handleDelete = async (id: string, planName: string) => {
    const confirmed = await confirmDialog({
      ...
      message: `Möchten Sie den Wochenplan "${planName}" wirklich löschen? Diese Aktion kann nicht rückgängig gemacht werden.`,
      confirmText: 'Löschen',
    ...
    await executeWithValidation(async () => {
      ...
      showNotification({
        type: 'success',
        title: 'Erfolg',
        message: 'Der Wochenplan wurde erfolgreich gelöscht.'
      });
    ...
    <div>
      ...
      {weeklyPlans.length === 0 ? (
        <div style={{
          ...
          <h3>Keine Wochenpläne vorhanden</h3>
          <p>Erstellen Sie Ihren ersten Wochenplan zur Zuweisung von Mitarbeitern zu Arbeitswochen!</p>
          {hasRole(['admin', 'maintenance']) && (
            <Link to="/weekly-plans/new">
              <button style={{
                marginTop: '15px',
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ color: '#666', fontSize: '14px', marginBottom: '10px' }}>
                  ...
                  <p style={{ margin: '5px 0 0 0' }}>
                    <strong>Wochen:</strong> {plan.weekCount} | <strong>Status:</strong> {getStatusBadge(plan.status)}
                  </p>
                </div>
      ...
      {!hasRole(['admin', 'maintenance']) && weeklyPlans.length > 0 && (
        <div style={{
          marginTop: '20px',
          padding: '15px',
          ...
          <strong>ℹ️ Informationen:</strong> Sie können Wochenpläne anzeigen und Ihre Präferenzen eintragen.
          Zum Erstellen neuer Pläne benötigen Sie Admin- oder Instandhalter-Berechtigungen.
        </div>
      )}
    </div>
  );
