...
export const DEFAULT_ZEBRA_SHIFTS: Omit<Shift, 'id' | 'planId'>[] = [
  ...
  ...Array.from({ length: 4 }, (_, i) => i + 1).flatMap(day => [
    { timeSlotId: 'morning', dayOfWeek: day, minEmployees: 1, maxEmployees: 2, color: '#3498db' },
    { timeSlotId: 'afternoon', dayOfWeek: day, minEmployees: 1, maxEmployees: 2, color: '#3498db' }
  ]),
  // Friday: Morning only
  { timeSlotId: 'morning', dayOfWeek: 5, minEmployees: 1, maxEmployees: 2, color: '#3498db' }
];
...
export const DEFAULT_SHIFTS: Omit<Shift, 'id' | 'planId'>[] = [
  ...
  ...Array.from({ length: 5 }, (_, i) => i + 1).flatMap(day => [
    { timeSlotId: 'morning', dayOfWeek: day, minEmployees: 1, maxEmployees: 2, color: '#3498db' },
    { timeSlotId: 'afternoon', dayOfWeek: day, minEmployees: 1, maxEmployees: 2, color: '#3498db' },
    { timeSlotId: 'evening', dayOfWeek: day, minEmployees: 1, maxEmployees: 2, color: '#3498db' }
  ])
];
